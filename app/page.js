...
const GLOBAL_CSS = `
  ...
  @supports (animation-timeline: view()) {
    ...
    .scroll-right { opacity:0; transform:translateX(40px);  animation:slideRight 0.6s ease forwards; animation-timeline:view(); animation-range:entry 0% entry 26%; }
    .scroll-scale { opacity:0; transform:scale(0.88);       animation:scaleIn    0.6s ease forwards; animation-timeline:view(); animation-range:entry 0% entry 26%; }
  }
    ...
    <header className="absolute top-0 left-0 right-0 z-50 w-full">
      <div className="mx-auto flex h-[68px] max-w-7xl items-center justify-between px-4 md:px-8 xl:px-12">
        <Link href="/" className="flex items-center no-underline">
          <Image src={logo} alt="Instroom" height={44} style={{ width: "auto" }} className="object-contain" />
        </Link>
      ...
      <div className="relative mx-auto max-w-7xl px-4 md:px-8 xl:px-12 flex flex-col justify-between" style={{ minHeight: "calc(100vh - 68px)" }}>
        ...
        <div className="flex flex-col-reverse md:flex-row items-center justify-between gap-8 py-10 md:py-16">
          <div className="w-full md:max-w-[520px] shrink-0 text-center md:text-left">
            ...
            <p className="fade-up mb-8 leading-relaxed text-white mx-auto md:mx-0" style={{ fontFamily: FI, fontSize: "clamp(0.9rem,2vw,1rem)", maxWidth: 420, animationDelay: "120ms" }}>
          ...
          <div className="fade-up flex w-full md:flex-1 items-center justify-center md:justify-end" style={{ animationDelay: "80ms" }}>
            <img src="/Hero.gif" alt="Instroom app preview" className="hero-img w-full object-contain" style={{ maxWidth: "clamp(260px,45vw,520px)", height: "auto" }} />
          </div>
        </div>
          ...
          <div className="brand-track-wrapper">
            <div className="brand-track">
              {[...BRANDS, ...BRANDS].map(({ name, src }, i) => (
                <div key={`${name}-${i}`} className="brand-item">
                  <Image src={src} alt={name} width={200} height={64} className="brand-logo" unoptimized />
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="mx-auto max-w-6xl px-4 md:px-8 xl:px-12">
        <div className="flex flex-col md:flex-row items-center justify-between gap-10 md:gap-16">
          <div className="scroll-left shrink-0 w-full md:w-auto flex justify-center">
            <Image src={thinkingImg} alt="How it works" className="how-img object-contain" width={320} height={320} style={{ width: "clamp(200px,40vw,320px)", height: "auto" }} />
          </div>
          <div className="flex-1 w-full">
            <div className="scroll-right">
              <h2 className="mb-8 font-bold text-white" style={{ fontFamily: FG, fontSize: "clamp(1.8rem,4vw,2.6rem)" }}>How Does It Work</h2>
            </div>
            <div className="flex flex-col">
              {HOW_STEPS.map(({ number, title, desc }, i, arr) => (
                <div key={number} className="scroll-left flex items-start gap-4" style={{ animationDelay: `${i * 100}ms` }}>
                  ...
                  <div className="pb-6">
                    <h3 className="mb-1 font-bold text-white" style={{ fontFamily: FG, fontSize: 17 }}>{title}</h3>
                    <p style={{ fontFamily: FI, fontSize: 14, color: "rgba(255,255,255,0.55)", lineHeight: 1.75 }}>{desc}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-16 gap-y-12 max-w-5xl mx-auto">
          {WHY_ITEMS.map(({ icon, title, desc }, i) => (
            <div key={title} className="scroll-scale neon-hover flex items-start gap-5 p-6" style={{ backgroundColor: "#0d0d0d", borderRadius: 16, animationDelay: `${i * 80}ms` }}>
              ...
              <div className="pt-1">
                <h3 className="mb-2 font-bold text-white" style={{ fontFamily: FG, fontSize: 18, lineHeight: 1.3 }}>{title}</h3>
                <p style={{ fontFamily: FI, fontSize: 13.5, color: "rgba(255,255,255,0.50)", lineHeight: 1.75 }}>{desc}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="divide-y divide-white/5 px-4">
            {CTA_STATS.map(({ label, value, green }) => (
              <div key={label} className="flex items-center justify-between py-2.5">
                <span style={{ fontFamily: FI, fontSize: 12, color: "rgba(255,255,255,0.55)" }}>{label}</span>
                <span className="font-semibold" style={{ fontFamily: FI, fontSize: 12, color: green ? "#4ade80" : "#fff" }}>{value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mx-auto max-w-6xl px-4 md:px-8 py-12 md:py-16 xl:px-12">
        <div className="scroll-fade neon-hover flex flex-col md:flex-row items-center justify-between gap-8 p-6 md:p-8" style={{ backgroundColor: "#111", borderRadius: 20, boxShadow: "0 4px 24px rgba(0,0,0,0.4)" }}>
          ...
          <div className="hidden md:flex flex-col items-center" style={{ maxWidth: 500 }}>
            <Image src={contactImg} alt="Contact" className="contact-img mb-4 object-contain" width={500} height={350} />
            <p className="text-center" style={{ fontFamily: FI, fontSize: 14, color: "rgba(255,255,255,0.7)", lineHeight: 1.75 }}>
      ...
      <div className="mx-auto max-w-6xl px-4 md:px-8 py-10 md:py-12 xl:px-12">
        <div className="flex flex-col gap-10 md:flex-row md:items-start md:justify-between">
          <div className="max-w-[220px]">
            <Image src={logo} alt="Instroom" height={44} style={{ width: "auto" }} className="mb-3 object-contain" />
            <p className="mb-5" style={{ fontFamily: FI, fontSize: 12, color: "rgba(255,255,255,0.45)", lineHeight: 1.7 }}>
